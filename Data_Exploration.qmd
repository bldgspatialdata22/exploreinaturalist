...
masternat_user_map <- ggplot() +
  mapboxapi::layer_static_mapbox(
    location = region_researchgrade,
    style_url = "mapbox://styles/mapbox/light-v10") +
  geom_sf(
    mapping = aes(color = user.login, shape = project),
    size = 2,
    alpha = 0.5,
    data = region_researchgrade
    ) +
...
masternat_user_map
ggsave("masternat__usermap.png", plot = masternat__user_map, path = "C:/Users/Kari/Documents/GitHub/exploreinaturalist/deliverables")
```
...
MD_rare_insects <- 
  st_semi_join(MD_rare_insects_inc, MD_insects, by = NULL, copy = FALSE, na_matches = c("na", "never"))


MD_rare_insects_map <- ggplot() +
  mapboxapi::layer_static_mapbox(
    location = MD_rare_insects,
    style_url = "mapbox://styles/mapbox/light-v10") +
  geom_sf(
    data = MD_rare_insects,
    aes(color = family),
    shape = 4,
    alpha = 0.25,
    size = 0.5
